...
[ApiController]
[Route("api/productos/fitness")]
[Authorize]
public class ProductosFitnessController(IProductoFitnessService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ProductoFitnessCreateDto dto)
    {
    ...
    [HttpPost("bulk")]
    public async Task<IActionResult> CreateBulk([FromBody] IEnumerable<ProductoFitnessBulkDto> dtos)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] ProductoFitnessUpdateDto dto)
    {
        var email = User.Identity?.Name ?? "Desconocido";
