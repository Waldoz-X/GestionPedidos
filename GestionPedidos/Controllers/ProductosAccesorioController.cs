...
[ApiController]
[Route("api/productos/accesorios")]
[Authorize]
public class ProductosAccesorioController(IProductoAccesorioService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ProductoAccesorioCreateDto dto)
    {
    ...
    [HttpPost("bulk")]
    public async Task<IActionResult> CreateBulk([FromBody] IEnumerable<ProductoAccesorioBulkDto> dtos)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] ProductoAccesorioUpdateDto dto)
    {
        var email = User.Identity?.Name ?? "Desconocido";
