...
[ApiController]
[Route("api/productos/espinillera")]
[Authorize]
public class ProductosEspinilleraController(IProductoEspinilleraService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ProductoEspinilleraCreateDto dto)
    {
    ...
    [HttpPost("bulk")]
    public async Task<IActionResult> CreateBulk([FromBody] IEnumerable<ProductoEspinilleraBulkDto> dtos)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] ProductoEspinilleraUpdateDto dto)
    {
        var email = User.Identity?.Name ?? "Desconocido";
