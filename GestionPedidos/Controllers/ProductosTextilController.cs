...
[ApiController]
[Route("api/productos/textil")]
[Authorize]
public class ProductosTextilController(IProductoTextilService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ProductoTextilCreateDto dto)
    {
    ...
    [HttpPost("bulk")]
    public async Task<IActionResult> CreateBulk([FromBody] IEnumerable<ProductoTextilBulkDto> dtos)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] ProductoTextilUpdateDto dto)
    {
        var email = User.Identity?.Name ?? "Desconocido";
