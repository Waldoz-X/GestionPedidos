...
public interface IProductoFitnessService
{
    Task<IEnumerable<ProductoFitnessDto>> ObtenerTodosAsync();
    Task<ProductoFitnessDto?> ObtenerPorIdAsync(Guid idProducto);
    Task<ProductoFitnessDto> CrearAsync(ProductoFitnessCreateDto dto, string userEmail);
    Task<ProductoFitnessDto?> ActualizarAsync(Guid idProducto, ProductoFitnessUpdateDto dto, string userEmail);
    Task<bool> EliminarAsync(Guid idProducto);
    Task<int> CrearMasivoAsync(IEnumerable<ProductoFitnessBulkDto> dtos, string userEmail);
}

public class ProductoFitnessService(AppDbContext dbContext) : IProductoFitnessService
{
    public async Task<IEnumerable<ProductoFitnessDto>> ObtenerTodosAsync()
    {
        var fitnessItems = await dbContext.ProductosFitness
        ...
        return fitnessItems.Select(MapToDto);
    }

    public async Task<ProductoFitnessDto?> ObtenerPorIdAsync(Guid idProducto)
    {
        var fitnessItem = await dbContext.ProductosFitness
            .Include(g => g.Producto)
                .ThenInclude(p => p.Variantes)
                    .ThenInclude(v => v.Skus)
            .AsNoTracking()
        ...
        return fitnessItem == null ? null : MapToDto(fitnessItem);
    }

    public async Task<ProductoFitnessDto> CrearAsync(ProductoFitnessCreateDto dto, string userEmail)
    {
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                var producto = new etProducto
                {
                    ...
                    NbArtefactoCrea = "ProductoFitnessService.CrearAsync"
                };

                dbContext.Productos.Add(producto);

                var fitnessItem = new etProductoFitness
                {
                    IdProducto = producto.IdProducto,
                    DsComposicion = dto.DsComposicion,
                    DsRelleno = dto.DsRelleno,
                    DsCierre = dto.DsCierre,
                    DsProteccion = dto.DsProteccion,
                    NoPesoOz = dto.NoPesoOz,
                    Producto = producto
                };

                dbContext.ProductosFitness.Add(fitnessItem);
                ...
                return await ObtenerPorIdAsync(producto.IdProducto) ?? MapToDto(fitnessItem);
            }
            catch (Exception)
            {
    ...
    public async Task<int> CrearMasivoAsync(IEnumerable<ProductoFitnessBulkDto> dtos, string userEmail)
    {
        ...
        var fitnessItems = new List<etProductoFitness>();
        ...
        foreach (var dto in dtos)
        {
            ...
            var producto = new etProducto
            {
                ...
                ClEstatusProducto = dto.ClEstatusProducto,
                ClOperadorCrea = userEmail,
                NbArtefactoCrea = "ProductoFitnessService.CrearMasivoAsync"
            };
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                ...
                dbContext.ProductosFitness.AddRange(fitnessItems);
    ...
    public async Task<ProductoFitnessDto?> ActualizarAsync(Guid idProducto, ProductoFitnessUpdateDto dto, string userEmail)
    {
        var fitnessItem = await dbContext.ProductosFitness
            .Include(g => g.Producto)
                .ThenInclude(p => p.Variantes)
        ...
        if (fitnessItem == null) return null;
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                fitnessItem.Producto.ClProducto = dto.ClProducto;
                fitnessItem.Producto.NbProducto = dto.NbProducto;
                fitnessItem.Producto.IdElemCategoria = dto.IdElemCategoria;
                fitnessItem.Producto.IdElemLineaColeccion = dto.IdElemLineaColeccion;
                fitnessItem.Producto.ClHsCode = dto.ClHsCode;
                fitnessItem.Producto.ClEstatusProducto = dto.ClEstatusProducto;
                fitnessItem.Producto.ClOperadorModifica = userEmail;
                fitnessItem.Producto.NbArtefactoModifica = "ProductoFitnessService.ActualizarAsync";
                fitnessItem.Producto.FeModificacion = DateTimeOffset.UtcNow;

                fitnessItem.DsComposicion = dto.DsComposicion;
                fitnessItem.DsRelleno = dto.DsRelleno;
                fitnessItem.DsCierre = dto.DsCierre;
                fitnessItem.DsProteccion = dto.DsProteccion;
                fitnessItem.NoPesoOz = dto.NoPesoOz;
                ...
                if (dto.Variantes != null)
                {
                    ...
                    var variantesAEliminar = fitnessItem.Producto.Variantes
                    ...
                    foreach (var inputVar in dto.Variantes)
                    {
                        ...
                        {
                            var existingVar = fitnessItem.Producto.Variantes.FirstOrDefault(v => v.IdVariante == inputVar.IdVariante.Value);
                            if (existingVar != null)
                            {
                                ...
                                existingVar.NbArtefactoModifica = "ProductoFitnessService.ActualizarAsync";
                                    ...
                                    foreach (var inputSku in inputVar.Skus)
                                    {
                                        if (inputSku.IdSku.HasValue && inputSkuIds.Contains(inputSku.IdSku.Value))
                                        {
                                            ...
                                            if (existingSku != null)
                                            {
                                                ...
                                                existingSku.NbArtefactoModifica = "ProductoFitnessService.ActualizarAsync";
                                            }
                                        }
                                        else
                                        {
                                            var nuevoSku = new etSku
                                            {
                                                ...
                                                ClOperadorCrea = userEmail,
                                                NbArtefactoCrea = "ProductoFitnessService.ActualizarAsync"
                                            };
                        ...
                        else
                        {
                            ...
                            {
                                IdVariante = Guid.NewGuid(),
                                IdProducto = fitnessItem.IdProducto,
    ...
    public async Task<bool> EliminarAsync(Guid idProducto)
    {
        ...
        if (fitnessItem == null) return false;
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                foreach (var variante in fitnessItem.Producto.Variantes)
                {
                ...
                dbContext.ProductosFitness.Remove(fitnessItem);
                dbContext.Productos.Remove(fitnessItem.Producto);
    ...
    private ProductoFitnessDto MapToDto(etProductoFitness g)
    {
